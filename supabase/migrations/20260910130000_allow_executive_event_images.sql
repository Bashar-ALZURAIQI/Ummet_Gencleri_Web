-- Every current executive role may create a published event, so every
-- executive role must be able to upload and register the image for their own
-- event. The gallery INSERT policy and register_managed_asset are updated in
-- lockstep. Ownership is derived from auth.uid() through executive_assignments
-- and each path stays owner-bound: events/<auth.uid()>/<asset-id>.<ext>.
-- All narrower per-folder permissions, owner-only cleanup, and the President's
-- broader authority remain unchanged.
...
BEGIN
  ...
  ELSIF asset_bucket = 'gallery' THEN
    ...
    IF NOT (
      ...
      OR (v_position IN ('ACADEMIC_HEAD', 'ACTIVITIES_HEAD') AND v_folder = 'documents')
