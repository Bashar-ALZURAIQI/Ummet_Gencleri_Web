-- The guide-document authorization migration intentionally narrowed documents
-- to protect the Student Guide, but its replacement policy/function omitted
-- the pre-existing events permission for several executive roles. Restore only
-- the event-image branch. Paths remain caller-owned and guide permissions stay
-- exactly as defined by the preceding migration.
  ...
  ELSIF asset_bucket = 'gallery' THEN
    ...
    ELSE
      IF NOT (
        ...
        OR (v_position IN ('VICE_PRESIDENT', 'FINANCE_HEAD', 'AUDIT_HEAD') AND v_folder IN ('events', 'documents'))
      ) THEN
