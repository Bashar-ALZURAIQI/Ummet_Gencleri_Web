-- Student Guide documents demand stricter server-side authorization than the
-- generic documents folder currently grants. Plan/report documents stay
-- reusable by VICE_PRESIDENT, FINANCE_HEAD, AUDIT_HEAD, ACADEMIC_HEAD and
-- ACTIVITIES_HEAD, while guide documents are restricted to PRESIDENT and
-- MEDIA_HEAD — the same roles that operate the StudentGuide editor.
--
-- Because generic `documents/<owner>/<asset-id>.<ext>` paths cannot distinguish
-- a plan/report from a guide document, guide files now use a narrow path marker:
--
--   documents/<owner-uuid>/guide/<asset-uuid>.<ext>
--
-- The gallery INSERT policy and register_managed_asset are updated in lockstep.
-- Ownership stays auth.uid()-derived and each guide path remains owner-bound, so
-- spoofing another user's folder is still denied. All unrelated permissions
-- (plan-document, report-document, event-image, news-image, gallery-image,
-- site-image, avatars) are preserved exactly.
    ...
    WHERE
      ...
      OR (
        ...
        AND (
          ...
          OR (
            (storage.foldername(name))[1] IN ('news', 'albums', 'site', 'videos')
            OR (storage.foldername(name))[1] = 'documents'
          )
        )
      )
      OR (
...
-- Guide document paths are validated against the MIME extension of the matched
-- kind so a spoofed `guide` marker cannot smuggle a non-guide file into the
-- folders that the same executive roles still own.
