...
export default function StudentGamificationPanel() {
  ...
  if (loading) return <div className="card p-6 text-center text-gray-500">{t('gamification.loading', 'جارٍ تحميل رتبتك ونقاطك...')}</div>;
  if (error) {
    return (
      <div className="card p-6 text-center text-rose-700">
        ...
        <button onClick={() => void refresh()} className="btn-secondary mt-3">
          ...
          {t('gamification.retry', 'إعادة المحاولة')}
        </button>
      </div>
    );
  }
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-3">
      <div className="card bg-gradient-to-l from-navy-800 to-navy-950 p-6 text-white">
        ...
        <p className="text-sm text-gray-300">{t('gamification.currentBalance', 'رصيد نقاطك الحالي')}</p>
      </div>
      <div className="card p-6">
        <Trophy className="h-7 w-7 text-gold-500" />
        <div className="mt-3 text-3xl font-extrabold text-navy-900">#{summary.rank}</div>
        <p className="text-sm text-gray-500">{t('gamification.rankTitle', 'ترتيبك بين الأعضاء')} {summary.isTopTen && `· ${t('gamification.topTen', 'ضمن أفضل 10')}`}</p>
      </div>
      <div className="card p-6">
        <Medal className="h-7 w-7 text-amber-500" />
        <div className="mt-3 text-xl font-extrabold text-navy-900">{tier.medal} {tier.label}</div>
        <p className="text-sm text-gray-500">{t('gamification.medalTitle', 'وسام العضوية الحالي')}</p>
      </div>

      <div className="card p-6 lg:col-span-3">
        <h3 className="mb-4 flex items-center gap-2 font-extrabold text-navy-900">
          <History className="h-5 w-5" />
          {t('gamification.historyTitle', 'سجل النقاط')}
        </h3>
        {summary.recentLedger.length === 0 ? (
          <p className="text-sm text-gray-500">{t('gamification.historyEmpty', 'لا توجد حركات نقاط بعد.')}</p>
        ) : (
          <div className="divide-y">
            {summary.recentLedger.map((entry) => (
              <div key={entry.id} className="flex items-center justify-between gap-4 py-4">
                <div className="min-w-0">
                  <p className="font-semibold text-gray-800">{t('gamification.reason', 'السبب: ')}{entry.reason}</p>
                  <p className="mt-1 flex items-center gap-1.5 text-sm text-gray-500">
                    <UserRoundCheck className="h-4 w-4 shrink-0" />
                    {t('gamification.by', 'بواسطة: ')}{ledgerCreatorPresentation(entry)}
                  </p>
                  <time className="mt-1 block text-xs text-gray-400">
                    {new Date(entry.createdAt).toLocaleString(localeCode)}
                  </time>
                </div>
