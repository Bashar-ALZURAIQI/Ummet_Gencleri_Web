...
export default function PublicRecognition() {
  ...
  const [star, setStar] = useState<MonthlyStar | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);

  useEffect(() => {
    void (async () => {
      const [l, s] = await Promise.all([loadPublicLeaderboard(), loadMonthlyStar()]);
      setLoading(false);
      if (!l.ok || !s.ok) {
        console.error('public recognition failed', !l.ok ? l.error : s.ok ? null : s.error);
        setError(true);
        return;
      }
      setLeaders(l.data);
  ...
  return (
    <section className="container-app py-14">
      <div className="mb-8 text-center">
        <span className="text-sm font-bold text-gold-600">{t('recognition.badge')}</span>
        <h2 className="mt-2 text-3xl font-extrabold text-navy-900">{t('recognition.title')}</h2>
      </div>
      {loading ? (
        <p className="text-center text-gray-500">{t('recognition.loading')}</p>
      ) : error ? (
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="card p-6 lg:col-span-2">
            ...
            {leaders.length === 0 ? (
              <p className="text-sm text-gray-500">{t('recognition.empty')}</p>
            ) : (
          ...
          <div className="card relative overflow-hidden bg-gradient-to-br from-gold-50 to-amber-100 p-6 text-center">
            ...
            {star ? (
              <>
                <UserAvatar
                  name={star.studentName}
                  avatarPath={star.avatarPath}
              ...
              <p className="mt-8 text-sm text-gray-600">{t('recognition.noMonthlyStar')}</p>
            )}
          </div>
        </div>
      )}
