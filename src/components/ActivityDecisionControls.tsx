...
export default function ActivityDecisionControls({
  ...
  if (!hasStudent) {
    return <button type="button" onClick={onLogin} className="btn-gold w-full">سجل الآن</button>;
  }
  if (access !== 'accepted' && !item?.canParticipate) {
    return (
      <button type="button" disabled className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-gold-200 bg-gold-50 px-4 py-2.5 text-sm font-semibold text-gold-700 disabled:cursor-not-allowed">
        <Lock className="h-4 w-4" /> انتظار قبول العضوية
      </button>
    );
  }
  if (loading) {
    return <button type="button" disabled className="w-full rounded-xl bg-gray-100 px-4 py-2.5 text-sm font-semibold text-gray-500">جارٍ تحميل حالة التسجيل...</button>;
  }
  if (!item) {
    return <button type="button" disabled className="w-full rounded-xl bg-gray-100 px-4 py-2.5 text-sm font-semibold text-gray-500">التسجيل غير متاح لهذا النشاط</button>;
  }
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs font-bold">
        ...
        <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 px-3 py-1 text-amber-700">
          ...
          {item.type === 'PAID' ? `التكلفة: ${item.pointsValue}` : `النقاط: ${item.pointsValue}`}
        </span>
        {item.economyExempt ? (
          <span className="rounded-full bg-emerald-50 px-3 py-1 text-emerald-700">معفى من نظام النقاط</span>
        ) : (
          <span className="rounded-full bg-emerald-50 px-3 py-1 text-emerald-700">رصيدك: {item.totalPoints}</span>
        )}
      </div>

      {state.reason === 'DEADLINE' && <p className="rounded-lg bg-gray-100 p-2 text-center text-xs font-bold text-gray-600">انتهى وقت التسجيل</p>}
      {state.reason === 'FULL' && <p className="rounded-lg bg-rose-50 p-2 text-center text-xs font-bold text-rose-700">مكتمل العدد</p>}
      {state.reason === 'POINTS' && <p className="rounded-lg bg-amber-50 p-2 text-center text-xs font-bold text-amber-800">نقاطك غير كافية</p>}
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        >
          <CheckCircle2 className="h-4 w-4" /> {busy ? 'جارٍ الحفظ...' : 'سأنضم'}
        </button>
        <button
          type="button"
        ...
        >
          <XCircle className="h-4 w-4" /> لن أنضم
        </button>
      </div>
    </div>
  );
