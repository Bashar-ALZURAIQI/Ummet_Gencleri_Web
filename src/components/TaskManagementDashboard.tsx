...
const completionLabels: Record<Exclude<TaskCompletionStatus, 'PENDING'>, string> = {
...
export default function TaskManagementDashboard() {
  ...
  const saveCompletion = async (
    ...
    notify('success', 'تم حفظ تقييم الطالب.');
  };

  const closeTask = async () => {
    if (!selectedTask) return;
    if (enrollments.some((row) => row.completionStatus === 'PENDING')) {
      notify('error', 'يجب تقييم جميع الطلاب قبل إغلاق المهمة.');
      return;
    }
    if (!confirm('سيتم إغلاق المهمة وتوزيع النقاط نهائياً. هل تريد المتابعة؟')) return;
    ...
    setEnrollments([]);
    notify('success', 'تم إغلاق المهمة وأرشفتها وتوزيع النقاط بنجاح.');
  };
    ...
    <section className="space-y-5" dir="rtl">
      ...
      <div className="card p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-extrabold text-navy-900">إدارة المهام</h2>
            <p className="mt-1 text-sm text-gray-500">
              راجع مهامك النشطة، قيّم المسجلين، ثم أغلق المهمة لتوزيع النقاط.
            </p>
          </div>
        ...
        {!error && !loadingTasks && tasks.length === 0 && (
          <div className="mt-5 rounded-2xl border border-dashed border-gray-200 p-10 text-center text-sm text-gray-500">
            لا توجد مهام نشطة ضمن صلاحياتك حالياً.
          </div>
        )}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${
                  ...
                  {task.status === 'FULL' ? 'مكتملة المقاعد' : 'مفتوحة'}
                </span>
              </div>
              ...
              <div className="mt-4 space-y-1.5 text-xs text-gray-600">
                <div className="flex items-center gap-2">
                  ...
                  {task.enrollmentCount} / {task.requiredStudents} مسجل
                </div>
                <div className="flex items-center gap-2">
                  <Clock3 className="h-4 w-4 text-navy-500" />
                  الإغلاق: {new Date(task.deadline).toLocaleString('ar-EG')}
                </div>
                <div>المكافأة: {task.pointsReward} نقطة</div>
                <div>أنشأها: {task.createdByName}</div>
              </div>
            </button>
          ))}
        </div>
      ...
      {selectedTask && (
        <div className="card p-6">
          ...
          {loadingEnrollments ? (
            <div className="py-10 text-center text-sm text-gray-500">جارٍ تحميل المسجلين...</div>
          ) : enrollments.length === 0 ? (
            <div className="mt-5 rounded-xl bg-gray-50 p-6 text-center text-sm text-gray-500">
              لا يوجد طلاب مسجلون في هذه المهمة. يمكنك إغلاقها وأرشفتها دون توزيع نقاط.
            </div>
          ) : (
            <div className="mt-5 space-y-2">
              {enrollments.map((row) => (
                ...
                >
                  ...
                  >
                    <option value="PENDING" disabled>اختر التقييم</option>
