...
export default function ExcuseReviewPanel() {
  ...
  const decide = async (row: PendingMandatoryExcuse, status: 'ACCEPTED' | 'PARTIAL' | 'REJECTED') => {
    ...
    notify('success', t('admin.excuses.reviewedSuccess', 'تم اعتماد تقييم العذر وحفظ حركة النقاط في السجل.'));
  };

  return <section className="card p-6">
    <TransientToast message={toast} onClose={() => setToast(null)} />
    <div className="mb-5 flex items-center justify-between gap-3">
      <div><h2 className="text-xl font-extrabold text-navy-900">{t('admin.excuses.title', 'إدارة الأعذار')}</h2><p className="text-sm text-gray-500">{t('admin.excuses.subtitle', 'أعذار الأنشطة الإلزامية التي تنتظر القرار.')}</p></div>
      <button type="button" onClick={() => void refresh()} className="btn-secondary" disabled={loading}><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /> {t('admin.excuses.refresh', 'تحديث')}</button>
    </div>
    {error && <div className="rounded-xl bg-rose-50 p-4 text-sm font-semibold text-rose-700">{error}</div>}
    {!loading && !error && rows.length === 0 && <div className="rounded-2xl border border-dashed border-gray-200 p-10 text-center text-gray-500">{t('admin.excuses.empty', 'لا توجد أعذار معلقة حالياً.')}</div>}
    <div className="space-y-4">{rows.map((row) => <article key={row.enrollmentId} className="rounded-2xl border border-gray-200 bg-white p-5">
      <div className="flex items-center gap-3"><UserAvatar name={row.studentName} avatarPath={row.avatarPath} className="h-11 w-11" /><div><h3 className="font-bold text-navy-900">{row.studentName}</h3><p className="text-sm text-gray-500">{row.activityTitle}</p></div><span className="mr-auto inline-flex items-center gap-1 rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700"><Clock3 className="h-3.5 w-3.5" /> {t('admin.excuses.pendingBadge', 'قيد الانتظار')}</span></div>
      <p className="mt-4 rounded-xl bg-gray-50 p-4 text-sm leading-7 text-gray-700">{row.excuseText}</p>
      <div className="mt-4 flex flex-wrap gap-2">
        <button disabled={busyId===row.enrollmentId} onClick={() => void decide(row,'ACCEPTED')} className="btn-primary bg-emerald-600 hover:bg-emerald-700"><CheckCircle2 className="h-4 w-4" /> {t('admin.excuses.actions.accept', 'عذر مقنع')}</button>
        <button disabled={busyId===row.enrollmentId} onClick={() => void decide(row,'PARTIAL')} className="btn-secondary"><ShieldCheck className="h-4 w-4" /> {t('admin.excuses.actions.partial', 'مقنع جزئياً (-5)')}</button>
        <button disabled={busyId===row.enrollmentId} onClick={() => void decide(row,'REJECTED')} className="inline-flex items-center gap-2 rounded-xl bg-rose-600 px-4 py-2 text-sm font-bold text-white"><XCircle className="h-4 w-4" /> {t('admin.excuses.actions.reject', 'غير مقنع (-15)')}</button>
      </div>
    </article>)}</div>
  </section>;
}
